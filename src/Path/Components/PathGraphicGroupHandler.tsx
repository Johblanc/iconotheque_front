...
export function PathGraphicGroupHandler(props: {
  ...
  const [current, setCurrent] = useState(
    new GraphiPoint({ x: 0, y: 0, ax: 0, ay: 0, bx: 0, by: 0 })
  );
  ...
  useEffect(() => {
    if (inModif.groupe !== -1 && inModif.item !== -1) {
      setCurrent(
        formOptions.translateGraphiPoint(
          graphyPoints[inModif.groupe].points[inModif.item]
        )
      );
    } else {
  ...
  const handleCurrent = (
    ...
    newPoints[inModif.groupe].points[inModif.item] =
      formOptions.untranslateGraphiPoint(newCurrent);
  ...
  const pointPosUp = (e: React.MouseEvent) => {
    ...
    newForm[inModif.groupe].points[inModif.item] =
      newForm[inModif.groupe].points[inModif.item - 1];
  ...
  const handleClose = (value: boolean) => {
    if (inModif.groupe !== -1) {
      const newPoints = [...graphyPoints].map((item) => item.copy);
      newPoints[inModif.groupe].isClose = value;
      setGraphyPoints(newPoints);
    }
  };
  ...
  return (
    <div>
                  ...
                  <g>
                    {group.points.map((point, j) => (
                      <path
                        ...
                        d={
                          point.status === "dur"
                            ? GraphiPoint.losange(j * 20 + 10, 7, 10, 10)
                            : point.status === "symétrique"
                            ? GraphiPoint.circle(j * 20 + 10, 7, 10, 10)
      ...
      <div className={APP_STYLE.PATH.GRAPH.GROUPHAND.GROUPBOX}>
        <h4
          className={`${
            inModif.groupe === -1 ? APP_STYLE.APP.VISUALDISABLED : ""
          }`}
        >
          {inModif.groupe === -1
            ? `Pas de Groupe sélectionné`
            : `Modification du Groupe ${inModif.groupe + 1}`}
        </h4>
        <EntriesCheck
          name="Fermer ce groupe"
          accecibilityId={"path-update-group-close"}
          defaultValue={
        ...
        <div className={APP_STYLE.PATH.GRAPH.GROUPHAND.BUTTONBOX}>
          ...
          <button
            ...
            disabled={
              inModif.groupe === -1 ||
              inModif.groupe === graphyPoints.length - 1
            }
          >
      ...
      <div className={APP_STYLE.PATH.GRAPH.GROUPHAND.GROUPBOX}>
        <h4
          className={`${
            inModif.item === -1 ? APP_STYLE.APP.VISUALDISABLED : ""
          }`}
        >
          {inModif.item === -1
            ? `Pas de Point sélectionné`
            : `Modification du Point ${inModif.item + 1}`}
        </h4>
        <div className={APP_STYLE.PATH.GRAPH.GROUPHAND.BUTTONBOX}>
          <button
            onClick={pointPosUp}
          ...
          <button
            ...
            disabled={
              inModif.item === -1 ||
              inModif.item === graphyPoints[inModif.groupe].points.length - 1
            }
          >
            Déplacer vers l'aval
          </button>
          ...
          <button
            ...
            disabled={
              inModif.item === -1 ||
              graphyPoints[inModif.groupe].points.length === 1
            }
          >
            Supprimer
          </button>
          ...
          <button
            ...
            disabled={
              inModif.item === -1 ||
              (inModif.item === 0 && inModif.groupe === 0)
            }
          >
        ...
        <div className={APP_STYLE.APP.DROPDOWN.BASE}>
          <button
            className={`${APP_STYLE.APP.DROPDOWN.BUTTON} ${
              inModif.item === -1 ? APP_STYLE.APP.VISUALDISABLED : ""
            }`}
            type="button"
          ...
          <ul
            className={APP_STYLE.APP.DROPDOWN.MENU}
            aria-labelledby="dropdownMenuButton1"
          >
            <li>
              <p
                className={APP_STYLE.APP.DROPDOWN.ITEM}
                onClick={() => handleStatus("dur")}
              >
                Point Dur
              </p>
            </li>
            <li>
              <p
                className={APP_STYLE.APP.DROPDOWN.ITEM}
                onClick={() => handleStatus("symétrique")}
              >
                Point Symétrique
              </p>
            </li>
        ...
        <div className={APP_STYLE.APP.XXXLFLEX}>
          <div>
            ...
            <div className={APP_STYLE.APP.ENTRY.DOUBLE.BOX}>
              <EntryNumber
                accecibilityId={"path-update-group-x"}
                name="x"
                ...
                setValue={(value?: number) => {
                  if (value !== undefined) {
                    handleCurrent("x", value);
                  }
                }}
              />
              <EntryNumber
                accecibilityId={"path-update-group-y"}
                name="y"
          ...
          <div>
            <h6
              className={`${
                inModif.item === -1 || current.status === "dur"
                  ? APP_STYLE.APP.VISUALDISABLED
                  : ""
              }`}
            >
              Vecteur aval
            ...
            <div className={APP_STYLE.APP.ENTRY.DOUBLE.BOX}>
              <EntryNumber
                accecibilityId={"path-update-group-aval-dx"}
                name="dx"
                ...
                setValue={(value?: number) => {
                  if (value !== undefined) {
                    handleCurrent("dbx", value);
                  }
                }}
              />
              <EntryNumber
                accecibilityId={"path-update-group-aval-dy"}
                name="dy"
          ...
          <div>
            <h6
              className={`${
                inModif.item === -1 || current.status !== "doux"
                  ? APP_STYLE.APP.VISUALDISABLED
                  : ""
              }`}
            >
            ...
            <div className={APP_STYLE.APP.ENTRY.DOUBLE.BOX}>
              <EntryNumber
                accecibilityId={"path-update-group-amont-dx"}
                name="dx"
                ...
                setValue={(value?: number) => {
                  if (value !== undefined) {
                    handleCurrent("dax", value);
                  }
                }}
              />
              <EntryNumber
                accecibilityId={"path-update-group-amont-dy"}
                name="dy"
