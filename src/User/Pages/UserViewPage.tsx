...
export function UserViewPage() : JSX.Element
{
  ...
  const SumPublished = pathPublic.filter(item => item.user.name === user.name).length ;
  const SumCreated = SumPublished + pathPrivate.length

  const icons = [...pathPublic.filter(item => item.user.name === user.name),...pathPrivate]
  return (
  <div>
    <AppHeader/>
    <AppNav actif={"profil"}/>
    ...
    <div>
      ...
      <p>Mail : {user.mail}</p>
      {SumCreated !== 0 && <p>Icônes créée{SumCreated > 1 && "s"} : {SumCreated} </p>}
      {SumPublished !== 0 && <p>Icônes publiée{SumPublished > 1 && "s"} : {SumPublished} </p>}
    </div>
    <div>
    ...
    <h3>Mes Icônes</h3>
    <div>
      {icons.map((item,i) => <IconItem path={item} key={i}/>)}
    </div>
